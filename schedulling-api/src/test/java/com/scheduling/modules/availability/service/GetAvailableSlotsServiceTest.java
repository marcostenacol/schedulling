...
class GetAvailableSlotsServiceTest {
    ...
    void shouldGenerateAvailableSlots() {
        ...
        when(availabilityRepository.findByProviderIdAndActiveTrue(providerId)).thenReturn(List.of(avail));
        when(blockRepository.findBlocksInRange(any(), any(), any())).thenReturn(Collections.emptyList());

        List<LocalTime> slots = getAvailableSlotsService.execute(new GetAvailableSlotsRequest(providerId, serviceId, date));
