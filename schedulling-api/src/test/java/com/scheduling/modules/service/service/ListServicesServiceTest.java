...
class ListServicesServiceTest {
  ...
  @InjectMocks private ListServicesService listServicesService;
  ...
  void shouldListActiveServices() {
    ...
    when(repository.findByProviderIdAndActiveTrue(eq(providerId), any()))
        .thenReturn(new PageImpl<>(List.of(service)));

    Page<ServiceResponseDTO> response =
        listServicesService.execute(new ListServicesService.Input(providerId, pageable));
