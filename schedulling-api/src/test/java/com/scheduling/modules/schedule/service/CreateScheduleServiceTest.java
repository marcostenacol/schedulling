...
class CreateScheduleServiceTest {
    ...
    void shouldCreateScheduleSuccessfully() {
        ...
        when(scheduleRepository.findOverlappingSchedules(any(), any(), any())).thenReturn(Collections.emptyList());
        when(blockRepository.findBlocksInRange(any(), any(), any())).thenReturn(Collections.emptyList());
        ...
        ScheduleResponseDTO response = createScheduleService.execute(new CreateScheduleService.Input(client, createDTO));
    ...
    void shouldFailWhenConflictExists() {
        when(serviceRepository.findById(createDTO.getServiceId())).thenReturn(Optional.of(service));
        when(userRepository.findById(createDTO.getProviderId())).thenReturn(Optional.of(provider));
        when(scheduleRepository.findOverlappingSchedules(any(), any(), any())).thenReturn(Collections.singletonList(new Schedule()));
