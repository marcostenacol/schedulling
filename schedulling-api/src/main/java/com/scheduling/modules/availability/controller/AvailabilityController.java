...
public class AvailabilityController extends BaseController {
  ...
  public ResponseEntity<ApiResponse<AvailabilityResponseDTO>> set(
      @AuthenticationPrincipal User user, @Valid @RequestBody AvailabilityDTO dto) {
    AvailabilityResponseDTO response =
        setAvailabilityService.execute(new SetAvailabilityService.Input(user, dto));
    return success("Disponibilidade configurada", response);
  }

  @GetMapping("/me")
  @PreAuthorize("hasRole('PROVIDER')")
  public ResponseEntity<ApiResponse<List<AvailabilityResponseDTO>>> listMe(
  ...
  public ResponseEntity<ApiResponse<Void>> block(
      @AuthenticationPrincipal User user, @Valid @RequestBody BlockAvailabilityDTO dto) {
