...
public class GetAvailableSlotsService implements BaseService<GetAvailableSlotsRequest, List<LocalTime>> {
    ...
    public List<LocalTime> execute(GetAvailableSlotsRequest input) {
        ServiceOffered service = serviceRepository.findById(input.getServiceId())
        ...
        int adjustedDay = input.getDate().getDayOfWeek().getValue() == 7 ? 0 : input.getDate().getDayOfWeek().getValue();

        List<Availability> availabilities = availabilityRepository.findByProviderIdAndActiveTrue(input.getProviderId());
