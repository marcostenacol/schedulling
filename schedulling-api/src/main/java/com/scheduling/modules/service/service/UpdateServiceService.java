...
public class UpdateServiceService implements BaseService<UpdateServiceRequest, ServiceResponseDTO> {
  ...
  public ServiceResponseDTO execute(UpdateServiceRequest input) {
    ...
    if (!service.getProvider().getId().equals(input.getProvider().getId())) {
      log.warn(
          "Tentativa de edição de serviço id={} por usuário sem permissão id={}",
          input.getServiceId(),
          input.getProvider().getId());
      throw new AppException(
          "Você não tem permissão para editar este serviço", HttpStatus.FORBIDDEN);
    }

    if (input.getData().getName() != null) service.setName(input.getData().getName());
    if (input.getData().getDescription() != null)
      service.setDescription(input.getData().getDescription());
    if (input.getData().getPrice() != null) service.setPrice(input.getData().getPrice());
    if (input.getData().getDurationMinutes() != null)
      service.setDurationMinutes(input.getData().getDurationMinutes());
