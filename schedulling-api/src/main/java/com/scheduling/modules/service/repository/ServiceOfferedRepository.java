...
public interface ServiceOfferedRepository extends JpaRepository<ServiceOffered, UUID> {
    ...
    @Query(value = "SELECT s FROM ServiceOffered s JOIN FETCH s.provider WHERE s.provider.id = :providerId AND s.active = true",
           countQuery = "SELECT count(s) FROM ServiceOffered s WHERE s.provider.id = :providerId AND s.active = true")
